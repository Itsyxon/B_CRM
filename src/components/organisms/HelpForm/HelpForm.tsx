...
const FormInput = ({ name, type = 'text', label, required, ...props }: {
    ...
    return (
        <div>
            ...
            <Input className="w-full" {...register(name, { required })} type={type} {...props} />
        </div>
    )
}
...
const FormTextarea = ({ name, label, required, ...props }: {
    ...
    return (
        <div className="flex-1 flex flex-col">
            {label && <Label required={required}>{label}</Label>}
            <TextArea className="w-full flex-1 min-h-[120px]" {...register(name, { required })} {...props} />
        </div>
    )
}
...
const HelpForm = ({ handleFormSubmit }: { handleFormSubmit: () => void }) => {
    ...
    const onSubmit = (data: HelpRequestType) => {
        // Language-neutral storage format: TYPE:value||BODY:text
        const newRequest: Request = {
            id: Date.now(),
            text: `TYPE:${data.problemType}||BODY:${data.problemText}`,
        }
    ...
    return (
        <div className="rounded-xl border border-[var(--border)] bg-[var(--tertiary)] p-6 w-full md:w-[380px] md:shrink-0">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                    <h2 className="text-base font-semibold text-[var(--secondary)]">{d.form.title}</h2>
                    <p className="text-xs text-[var(--accent-gray)] mt-0.5">{d.form.subtitle}</p>
                </div>
            </div>
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 gap-4">
                        <FormInput name="name" label={d.form.nameLabel} placeholder={d.form.namePlaceholder} required />
                        <FormInput name="email" type="email" label={d.form.emailLabel} placeholder="ivan@company.com" required />
                    </div>

                    <FormSelect
                        name="problemType"
                        label={d.form.typeLabel}
                        placeholder={d.form.typePlaceholder}
                        options={d.form.problemTypes}
                    />

                    {problemType === 'other' && (
                        <FormInput
                            name="reason"
                            label={d.form.reasonLabel}
                            placeholder={d.form.reasonPlaceholder}
                            required
                        />
                    )}
